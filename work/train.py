...
if __name__ == "__main__":
    ...
    for test_id in range(len(users)):
        if users[test_id][-1] == '2':
            ...
            FPs.append(FN)
            FNs.append(FP)
            neg = float(sum(np.array(Y_test) == 0)) / len(Y_test)
            print(users[test_id], acc, neg)
            accs.append(acc)
            negs.append(neg)
            #if users[test_id] == 'hfy2':
            #    pickle.dump([scalar, clf], open('model/tap.model', 'wb'))
    print('Error Rate =', 1.0 - np.mean(accs), np.std(accs), np.mean(negs), np.std(negs))
    print(np.mean(FPs), np.std(FPs))
    print(np.mean(FNs), np.std(FNs))
