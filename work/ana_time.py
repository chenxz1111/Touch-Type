...
aov = pg.rm_anova(dv='select', within=['board', 'session'], 
                  subject='user',data=df,correction=False,
             detailed=True, effsize='np2')

print(aov)

aov = pg.rm_anova(dv='delete', within=['board', 'session'], 
                  subject='user',data=df,correction=False,
             detailed=True, effsize='np2')

print(aov)

aov = pg.rm_anova(dv='others', within=['board', 'session'], 
                  subject='user',data=df,correction=False,
             detailed=True, effsize='np2')
